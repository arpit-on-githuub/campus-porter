...
Backend goes to Koyeb, which has a free always-on tier that supports WebSockets. Push the repo to GitHub, create a service from it, set the work directory to `backend`, use the Dockerfile builder, add the environment variables, and set the health check path to `/`.

Frontend stays on Vercel as a static Vite build (`npm run build` produces `dist/`). After the backend moves, update `VITE_API_URL` to the Koyeb URL and redeploy.

The database stays on MongoDB Atlas. Before going live, open Network Access and allow the backend host to connect, and turn on backups.
...
**Phase 5, going properly live (in progress).** Moving the backend off the Railway trial onto an always-on host, wiring up a real email provider, and turning the web app into an installable mobile app.
